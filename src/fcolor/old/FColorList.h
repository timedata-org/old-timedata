...
class FColorList : public std::vector<FColor> {
  public:
    FColor& at(size_t i) {
        DCHECK(isIndex(i)) << "Bad index " << i;
        return std::vector<FColor>::at(i);
    }
    ...
    const FColor& get(size_t i) const {
        return isIndex(i) ? std::vector<FColor>::at(i) : FColor::BLACK;
    }
    ...
    void resize(unsigned long s) {
        std::vector<FColor>::resize(s, FColor::BLACK);
    }
